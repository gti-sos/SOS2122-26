...
module.exports.register = (app) => {
    ...
    app.get(BASE_API_PATH, (req,res)=>{
        ...
        if (req.query.installed_capacity_mw) dbquery["installed_capacity_mw"] = parseInt(req.query.installed_capacity_mw);
        if (req.query.generation_gwh) dbquery["generation_gwh"] = parseInt(req.query.generation_gwh);
        if (req.query.renovable_inst_cap_mw) dbquery["renovable_inst_cap_mw"] = parseInt(req.query.renovable_inst_cap_mw);
        if (req.query.renovable_gen_gwh) dbquery["renovable_gen_gwh"] = parseInt(req.query.renovable_gen_gwh);
        if (req.query.renovable_percentage) dbquery["renovable_percentage"] = parseFloat(req.query.renovable_percentage);
        if (req.query.var) dbquery["var"] = parseFloat(req.query.var);

        //Búsqueda de datos y ordenación por parametro country
        db.find(dbquery).sort({country:1, year:-1}).skip(offset).limit(limit).exec((error, dataElectricity) => {
            if (error){
